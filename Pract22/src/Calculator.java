...
public class Calculator extends JFrame {
    ...
    public Calculator() {
        ...
        buttonSpace.addActionListener( e -> {
            input.append(" ");
            outputArea.setText(input.toString());
        });
        buttonRes.addActionListener(e -> {
            try {
                ArrayList<String> list = new ArrayList<>(Arrays.asList(input.toString().replaceAll("\\s+", " ").split(" ")));
    ...
    private String calc(ArrayList<String> list) {
        ...
        for (String operand : list) {
            switch (operand) {
                ...
                default -> {
                    stack.push(Float.parseFloat(operand));
                }
            }
        }
        if (stack.size() != 1)
